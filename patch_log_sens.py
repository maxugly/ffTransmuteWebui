...
old_down_fill = """    document.getElementById('kstLabel').textContent = `Sens: ${currentSens.toFixed(1)}%`;
    document.getElementById('kstFill').style.width = `${(currentSens / 300.0) * 100}%`;"""

new_down_fill = """    document.getElementById('kstLabel').textContent = `Sens: ${currentSens.toFixed(1)}%`;
...
old_move = """  function onMouseMove(e) {
...
new_move = """  function onMouseMove(e) {
    const deltaX = e.clientX - startX;
    // Logarithmic scale: S_new = S_old * exp(deltaX * factor)

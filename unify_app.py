...
# updateGlobalInputs rewrite
old_update = r'function updateGlobalInputs\(\) \{.*?(?=function _syncTabInputFromGlobal)'
...
"""
js = re.sub(old_update, new_update, js, flags=re.DOTALL)

# updateStatusIndicators rewrite
old_status = r'function updateStatusIndicators\(\) \{.*?(?=\n\}\n)'
...
"""
js = re.sub(old_status, new_status, js, flags=re.DOTALL)

# syncGlobalPanelVisibility rewrite
old_sync = r'function syncGlobalPanelVisibility\(\) \{.*?(?=\n\}\n)'
...
"""
js = re.sub(old_sync, new_sync, js, flags=re.DOTALL)

# Listeners rewrite
old_listeners = r'var btnQuickVIn = document\.getElementById\(\'btnQuickVIn\'\);.*?var btnToggle = document\.getElementById\(\'btnGlobalToggle\'\);'
...
js = re.sub(old_listeners, new_listeners, js, flags=re.DOTALL)

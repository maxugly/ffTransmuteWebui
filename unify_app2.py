...
def repl_update(m):
    return """function updateGlobalInputs() {
...
js = re.sub(r'function updateGlobalInputs\(\) \{.*?(?=function _syncTabInputFromGlobal)', repl_update, js, flags=re.DOTALL)

def repl_status(m):
    return """function updateStatusIndicators() {
...
js = re.sub(r'function updateStatusIndicators\(\) \{.*?(?=\n\}\n)', repl_status, js, flags=re.DOTALL)

def repl_sync(m):
    return """function syncGlobalPanelVisibility() {
...
js = re.sub(r'function syncGlobalPanelVisibility\(\) \{.*?(?=\n\}\n)', repl_sync, js, flags=re.DOTALL)

def repl_listeners(m):
    return """  var btnQuickI = document.getElementById('btnQuickI');
  if (btnQuickI) {
...
js = re.sub(r'var btnQuickI = document\.getElementById\(\'btnQuickI\'\);.*?var btnToggle = document\.getElementById\(\'btnGlobalToggle\'\);', repl_listeners, js, flags=re.DOTALL)
js = re.sub(r'var btnQuickVIn = document\.getElementById\(\'btnQuickVIn\'\);.*?var btnToggle = document\.getElementById\(\'btnGlobalToggle\'\);', repl_listeners, js, flags=re.DOTALL)

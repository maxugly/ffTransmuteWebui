...
|------|-------|---------|------------|----------|
| `media_store.py` | 1,324 | media cache, thumbnails, pool state, project persistence, frame extraction | pathutil, shell | **9/10** — four concerns in one file |
...
| `main.py` | 299 | app creation, middleware, ops loop, startup | all routes, operations, media_store | 3/10 — was 700, now thin |
...
| `datamosh_ops.py` | 796 | melt, classic, hijack, destruct, mv_hack — 5 mosh modes | shell, bin/ | **7/10** — five handlers, shared helpers, could split per mode |
| `deepdream_ops.py` | 392 | deepdream handler, param model, UI wiring | deepdream_engine, contract | 4/10 — handler is fine, engine is the problem |
| `transmute_ops.py` | 352 | crop, stretch, extract, join, grid, fit, raw — 8 sub-ops | shell, pathutil | 5/10 — many sub-ops but each is small |
...
| `facemorph_engine.py` | 295 | dlib landmarks, delaunay, batch morph | dlib, PIL, png_pipeline | 4/10 — pipeline extracted |
...
| `rife_ops.py` | 201 | handler, ffprobe, rife-ncnn-vulkan subprocess | shell, png_pipeline | 2/10 — pipeline extracted, thin |
...
|------|-------|-----------|----------|
| `picker.py` | 188 | 1 (kdialog/zenity/tkinter) | 4/10 — three backends, but one purpose |
| `media.py` | 106 | 8 (video, image, probe, media_info, thumbnail, hash, export, cache) | 3/10 — grouped well |
...
|------|-------|---------|----------|
| `app.js` | 7,620 | ALL UI logic — tabs, forms, pool, file browser, API calls | **10/10** — the mother of all monoliths |
| `style.css` | 3,646 | ALL styling — layout, components, operations, pool | **9/10** — one file, 3600 lines |
